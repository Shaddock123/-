...
class Trainer(object):
    def __init__(self, weight_path, resume, gpu_id):
        ...
        # YOLOv3的损失函数，包括矩形框中心点误差，预测框宽高误差，预测框置信度损失，预测框类别损失
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            with torch.no_grad():
                APs, miou = Evaluator(self.yolov3).APs_voc()  # 得到评价指标mAP， mIoU
                for i in APs:
                ...
                self.writer.add_scalar("train loss", np.mean(loss_all), epoch)
                self.writer.add_scalar("val loss", np.mean(loss_val), epoch)
